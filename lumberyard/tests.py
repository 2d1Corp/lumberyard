...
class ToggleMaterialReplenishmentViewTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        category = Category.objects.create(name="Boards")
        ...
        warehouse_a = Warehouse.objects.create(name="Warehouse A")
        warehouse_b = Warehouse.objects.create(name="Warehouse B")
        cls.stock_a = StockBalance.objects.create(
            material=cls.material,
            warehouse=warehouse_a,
            quantity=10,
        )
        cls.stock_b = StockBalance.objects.create(
            material=cls.material,
        ...
        cls.url = reverse(
            "material-toggle-all-replenishment",
    ...
    def test_get_does_not_replenish(self):
        response = self.client.get(self.url)

        self.stock_a.refresh_from_db()
        self.stock_b.refresh_from_db()
        self.assertEqual(response.status_code, 405)
        self.assertIsNone(self.stock_a.replenishment_requested_at)
        self.assertIsNone(self.stock_b.replenishment_requested_at)

    def test_post_replenish_all_stocks(self):
        response = self.client.post(self.url)

        self.stock_a.refresh_from_db()
        self.stock_b.refresh_from_db()
        self.assertEqual(response.status_code, 302)
        self.assertIsNotNone(self.stock_a.replenishment_requested_at)
        self.assertIsNotNone(self.stock_b.replenishment_requested_at)
        self.assertEqual(self.stock_a.replenishment_requested_by, self.user)
        self.assertEqual(self.stock_b.replenishment_requested_by, self.user)
